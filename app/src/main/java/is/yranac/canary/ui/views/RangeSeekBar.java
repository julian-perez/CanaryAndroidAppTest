...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private final T absoluteMinValue, absoluteMaxValue, minRange;
    private final NumberType numberType;
    private final double absoluteMinValuePrim, absoluteMaxValuePrim, minRangePrim;
    private double normalizedMinValue = 0d;
    private double normalizedMaxValue = 1d;
    private double normalizedBound;
     ...
     * @param thumbNormal_ResID The ResourceID of the image for the pressed thumb icon.
     ...
     */
    public RangeSeekBar(T absoluteMinValue, T absoluteMaxValue, T minRange,
                        Context context, int leftColor, int middleColor, int rightColor,
                        int thumbNormal_ResID)
        ...
        minRangePrim = minRange.doubleValue();
        normalizedBound = normalizedBound(minRange);
        ...
        RIGHT_COLOR = rightColor < 0 ? rightColor : Color.argb(0xFF, 0x00,
                0x00, 0xFF);
        thumbImage = getResources().getDrawable(
                thumbNormal_ResID);
        thumbWidth = thumbImage.getIntrinsicWidth();
        thumbHalfWidth = 0.5f * thumbWidth;
        thumbHalfHeight = 0.5f * thumbImage.getIntrinsicHeight();
        lineHeight = DensityUtil.dip2px(getContext(), 3);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (!IS_MULTI_COLORED) {
            ...
            final RectF rect = new RectF(lineHeight * 0.5f,
                    0.5f * (getHeight() - lineHeight), getWidth() - lineHeight * 0.5f,
            ...
            final RectF rectR = new RectF(lineHeight * 0.5f,
                    0.5f * (getHeight() - lineHeight),
                    normalizedToScreen(normalizedMinValue),
                    0.5f * (getHeight() + lineHeight));
            paint.setColor(LEFT_COLOR);
            canvas.drawRoundRect(rectR, padding, padding, paint);

            // draw seek bar background line
            final RectF rectY = new RectF(lineHeight * 0.5f,
                    0.5f * (getHeight() - lineHeight), getWidth() - lineHeight * 0.5f,
                    0.5f * (getHeight() + lineHeight));
            ...
            canvas.drawRoundRect(rectY, padding, padding, paint);
            ...
            canvas.drawRoundRect(rectG, padding, padding, paint);
        }
    ...
    private void drawThumb(float screenCoord, Canvas canvas) {
        ...
        canvas.save();
        canvas.translate(screenCoord
                - thumbHalfWidth, (0.5f * getHeight()) - thumbHalfHeight);

        thumbImage.setBounds(0, 0, thumbImage.getIntrinsicWidth(), thumbImage.getIntrinsicHeight());
    ...
    public void setNormalizedMinValue(double value) {
        normalizedMinValue = Math.max(0d,
                Math.min(1d, Math.min(value, normalizedMaxValue - normalizedBound)));
        invalidate();
    }
    ...
    public void setNormalizedMaxValue(double value) {
        normalizedMaxValue = Math.max(0d,
                Math.min(1d, Math.max(value, normalizedMinValue + normalizedBound)));
        invalidate();
    }
    ...
    private double normalizedBound(T value) {

        if (value.doubleValue() > absoluteMaxValuePrim - absoluteMinValuePrim)
            return 0d;

        return value.doubleValue() / (absoluteMaxValuePrim - absoluteMinValuePrim);
    }
